...
* I managed to do part 2, got a lot of inspiration from Adam K's solution, although with my input, the first state repeat happened when spawning the 1705th block, which had the same state as the 5th block (also a period of 1700, I don't think it's a coincidence). The height increase between block 5 and 1705 however was too big to be realized also in later iterations (I guess the board was fuller then? but how can that cause the height increase become smaller?), but when I found the *second* repeat of this state (at block 3420 for my input), then it somehow gave the right answer.
I can't really say I fully understand *why* this works: even if the moves and the blocks are the same, it seems like the *board* may change a lot between block n and block n + 1700.... also in terms of my coding style this definitely feels like the most hacky one so far (I ended up writing out my entire reasoning as formatted prints) but I was only focused on getting that second star. Now I want to go look a bit at how other people solved this. 
* My solution totally does not work on the example input (but the version where I looked immediately at the first state repeat did) and I now realize I totally got lucky that secondtime % 5 == p which I never checked...